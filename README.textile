...
connection.use("select facebookid from friends") { row =>
  // row holds all the secrets to the current row
  SpamMachine sendFreeGiftTo Friend(row.int(0))
}
</pre>
...
// drinks is a List[Espresso] from many well-known Melbourne cafes. Mint!
</pre>
